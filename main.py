...
class ApplicationController:
    ...
    def _initialize_components(self):
        ...
        try:
            ...
            if cap.isOpened():
                ...
                try:
                    ...
                    print(
                        f"Błąd podczas inicjalizacji EyeTracker: {type(e).__name__}: {e}"
                    )
                    self.gaze_tracker_instance = None
                    self.eye_monitor_worker = None
            else:
        ...
        # Pokaż główne okno od razu po uruchomieniu
        self.settings_window.show()
        self.settings_window.raise_()  # Upewnij się, że okno będzie na wierzchu
        self.settings_window.activateWindow()  # Aktywuj okno
    ...
    def resume_main_timer(self):
        ...
        if (
            self.current_state == self.STATE_WORKING
            and not self.main_work_timer.isActive()
        ):
    ...
    def _create_enhanced_main_page(self):
        ...
        self.background_music_player.mediaStatusChanged.connect(
            self._debug_background_status
        )

        if os.path.exists(self.audio_file_path):
            self.media_player.setMedia(
                QMediaContent(QUrl.fromLocalFile(self.audio_file_path))
            )
        else:
            print(f"Błąd: Plik audio '{self.audio_file_path}' nie został znaleziony.")

        if os.path.exists(self.background_music_path):
            self.background_playlist.addMedia(
                QMediaContent(QUrl.fromLocalFile(self.background_music_path))
            )
            ...
            print(
                f"Błąd: Plik muzyki '{self.background_music_path}' nie został znaleziony."
            )
    ...
    def _setup_main_gif_player(self):
        ...
        if not self.gif_movie.isValid():
            print(
                f"Błąd: Plik GIF '{gif_file}' nie jest prawidłowym plikiem GIF lub nie został znaleziony."
            )
            self.gif_display_label.setText(
                "BŁĄD: Nie znaleziono GIF-a lub jest uszkodzony."
            )
    ...
    def _stop_main_session(self):
        ...
        if (
            hasattr(self, "background_music_player")
            and self.background_music_player.state() == QMediaPlayer.PlayingState
        ):
        ...
        # Stop GIF
        if (
            hasattr(self, "gif_movie")
            and self.gif_movie.isValid()
            and self.gif_movie.state() == QMovie.Running
        ):
            self.gif_movie.stop()
    ...
    def _update_countdown(self):
        ...
        if self.current_seconds_left <= 0:
            ...
            self.status_label.setStyleSheet(
                "font-size: 20px; font-weight: bold; color: #4c6ef5;"
            )
    ...
    def _on_instruction_finished(self, status):
        ...
        if status == QMediaPlayer.EndOfMedia:
            ...
            if os.path.exists(self.background_music_path):
                ...
                print(
                    f"Rozmiar pliku: {os.path.getsize(self.background_music_path)} bajtów"
                )

                # Sprawdź stan odtwarzacza przed próbą odtwarzania
                print(
                    f"Stan background player przed play(): {self.background_music_player.state()}"
                )
                print(
                    f"Status background player przed play(): {self.background_music_player.mediaStatus()}"
                )

                # Próba ręcznego ustawienia media przed odtwarzaniem
                self.background_music_player.setMedia(
                    QMediaContent(QUrl.fromLocalFile(self.background_music_path))
                )
    ...
    def resume_main_break_timer(self):
        ...
        if self.is_main_paused:
            ...
            self.status_label.setStyleSheet(
                "font-size: 20px; font-weight: bold; color: #51cf66;"
            )
            ...
            if hasattr(self, "bpm_label") and self.bpm_label is not None:
                if bpm is None:
                    ...
                    try:
                        ...
                        if (
                            self.current_state == self.STATE_BREAK
                            and hasattr(self, "session_heartbeat_data")
                            and 40 <= bpm_value <= 200
                        ):  # filtruj nieprawdopodobne wartości
                            self.session_heartbeat_data.append(bpm_value)
                            print(
                                f"Zapisano tętno: {bpm_value} BPM (łącznie: {len(self.session_heartbeat_data)} pomiarów)"
                            )
    ...
    def _save_break_session_data(self):
        ...
        try:
            ...
            print(
                f"   - Średnie tętno: {sum(heartbeat_data) / len(heartbeat_data):.1f} BPM"
            )
